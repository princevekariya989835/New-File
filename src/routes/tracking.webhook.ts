...
export const Route = createFileRoute("/tracking/webhook")({
  server: {
    handlers: {
      POST: async ({ request }: { request: Request }) => {
        try {
          ...
          console.log(`[Zippyy Tracking Webhook] Received ${event} for Order: ${orderId}, AWB: ${awb}, Status: ${status}`);
          ...
          if (awb || orderId) {
            ...
            try {
              ...
              console.error("[Zippyy Tracking Webhook] Database update error:", dbErr);
            }
          }
          ...
          console.error("[Zippyy Tracking Webhook] Unhandled exception:", err);
          return new Response(
